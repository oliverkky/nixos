...
let
  vcvRackPackage = vcv-rack.overrideAttrs (_old: {
    # nixpkgs fetches a deleted upstream PR patch on Linux.
    # Drop this once pkgs.vcv-rack no longer references PR 1944.
    patches = [
      (path + "/pkgs/by-name/vc/vcv-rack/rack-minimize-vendoring.patch")
      (writeText "vcv-rack-fix-plugin-build-order.patch" ''
...
symlinkJoin {
  ...
  postBuild = ''
    rm "$out/bin/Rack"
    makeWrapper "${pipewire.jack}/bin/pw-jack" "$out/bin/Rack" \
